...
import { getDoc, doc, addDoc, deleteDoc, collection, serverTimestamp, getDocs, updateDoc, query, where, orderBy } from 'firebase/firestore';
...
const Profile = () => {
  ...
  const [showOffensive, setShowOffensive] = useState(false);
  const emailRef = useRef();
  const passwordRef = useRef();

  const checkFeedback = async (text) => {
    try {
      const response = await fetch('http://192.168.100.91:5000/check-feedback', {
        method: 'POST',
  ...
  const sendFeedback = async () => {
    ...
    const isOffensive = await checkFeedback(content);
    try {
  ...
  const sendReply = async (id) => {
    ...
    try {
      ...
      Alert.alert('Success', 'Răspuns trimis!');
  ...
  return (
    <ScreenWrapper>
      ...
      <View style={styles.form}>
        {isGuest ? (
          <Text style={styles.text}>Please login to access all the features.</Text>
        ) : showFeedbackPanel ? (
          <>
            <Pressable onPress={() => setShowOffensive(!showOffensive)}>
              <Text style={[styles.text, { textAlign: 'center', marginBottom: 10, color: theme.colors.primaryDark }]}> 
                {showOffensive ? 'Hide offensive feedback' : 'Show offensive feedback'}
              </Text>
            </Pressable>
            <FlatList
              data={feedbackList.filter(item => showOffensive || !item.offensive)}
              ...
              renderItem={({ item }) => (
                <View style={styles.feedbackCard}>
                  ...
                  />
                  <ButtonC title="Reply" onPress={() => sendReply(item.id)} style={styles.button} />
                  <ButtonC
                    title="Delete"
                    onPress={async () => {
                      try {
                        ...
                        Alert.alert('Deleted', 'Feedback deleted.');
                      } catch (err) {
          ...
          <>
            ...
            {isAdmin && !showFeedbackPanel && <Pressable onPress={() => setShowFeedbackPanel(true)}><Text style={[styles.text, { color: theme.colors.primaryDark, marginTop: 10 }]}>View feedback</Text></Pressable>}
            {!isAdmin && <Pressable onPress={() => setFeedbackMode(true)}><Text style={[styles.text, { color: theme.colors.primaryDark, marginTop: 10 }]}>Feedback or problems</Text></Pressable>}
...
const styles = StyleSheet.create({
  header: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: wp(5), paddingVertical: hp(1.25) },
  iconWrapper: { padding: wp(1.25), borderRadius: theme.radius.sm },
  form: { width: '100%', gap: hp(1.875), marginTop: hp(1.25), paddingHorizontal: wp(5) },
  text: { fontSize: wp(4.5), textAlign: 'center', color: theme.colors.text },
  button: { height: hp(5), alignSelf: 'flex-start' },
  inputText: { borderWidth: 1, borderColor: '#ccc', borderRadius: 5, padding: 10, marginBottom: 10, fontSize: wp(4), backgroundColor: '#fff' },
  feedbackCard: { backgroundColor: '#f9f9f9', padding: 15, borderRadius: 8, marginBottom: 15 },
  feedbackTitle: { fontWeight: 'bold', fontSize: wp(4.5), marginBottom: 5 },
  showPasswordIcon: { position: 'absolute', right: wp(2.5), top: '50%', transform: [{ translateY: -13 }] },
});
